"""
viewforge_core - turn a multi-view blueprint sheet into verified orthographic
reference planes, plus the numbers needed to place them in Blender.
...
Two ways to define a view:
  ...
  source='box'        You draw the rectangle, and the rectangle IS the
                      ...
                      outside is clipped. Use this on sheets where dimension
                      lines join the views together, which defeats auto-detect.

No GUI here - see viewforge_gui.py.
"""
...
# horizontal axis, 'v' the vertical. L=length, W=width, H=height.
ROLE_AXES = {
    "side":   ("L", "H"),
    "top":    ("L", "W"),
    "bottom": ("L", "W"),
...
# 'facing' = which edge of the image the object's front sits against.
BLENDER = {
...
# ------------------------------------------------------------ sheet i/o ----
def load_sheet(path, threshold=None):
    """Return (grayscale, ink mask, threshold used)."""
    g = np.asarray(Image.open(path).convert("L")).astype(np.int16)
    if threshold is None:
        paper = np.percentile(g, 90)          # blueprints are mostly paper
...
def detect_views(ink, gap=9, min_side=60):
    """Group ink into candidate views by dilating until nearby strokes merge."""
...
def segmentation_quality(views, ink):
    """Warn when auto-detect has clearly failed - e.g. dimension lines have
    welded several views into one giant region."""
    h, w = ink.shape
    for v in views:
        if (v["x1"] - v["x0"]) > 0.9 * w and (v["y1"] - v["y0"]) > 0.35 * h:
            return ["Auto-detect merged a large area into one region. This "
                    "sheet's dimension lines probably join the views together. "
                    "Switch to manual boxes: drag a rectangle over each view "
                    "with its edges on the object's extremes."]
    return []


# ------------------------------------------------- per-view measurement ----
def clamp_box(v, shape):
    """Put a hand-drawn box in order and inside the sheet.

    This matters more than it looks. numpy slices silently, so an x1 typed
    past the right-hand edge used to trim the MASK but leave the reported span
    at the typed value - handing solve() a scale several times too small with
    no visible sign of it. The box is the measurement, so it has to be a box
...
def view_metrics(v, ink, lab):
    """Return (mask, span_h, span_v, box) for one view, honouring its source."""
    if v["source"] == "component" and v["cid"]:
        if lab is None:
            raise SolveError(
                "A view still refers to an auto-detected region, but the sheet "
                "has not been analysed. Press Detect views.")
        mask = ink & (lab == v["cid"])
        ...
        if x1 - x0 < 2 or y1 - y0 < 2:
            raise SolveError(
                ...
                f"sheet. Drag it again with its edges on the object's extremes.")
...
def symmetry_axis(mask, box, axis):
    ...
    The candidate axis is confined to a narrow band around the bounding-box
    midpoint, because a symmetric object's mirror line cannot be far from its
    own centre. Every candidate is scored over the SAME comparison width, so a
    near-edge axis with only a few overlapping samples cannot win by comparing
    almost nothing - scoring by mean difference over a variable width was the
    original bug, where thin low-ink tails looked near-identical and beat the
    true axis by tens of pixels. And the winner has to be a clear improvement
    on the plain midpoint, so a near-tie on noise leaves the midpoint alone.

    The search band and the accept threshold are the same number. They used to
    differ, which made the outer part of the search unreachable: anything found
    there was thrown away rather than used.
    """
    x0, x1, y0, y1 = box
    ...
    while c <= mid + band + 1e-9:
        ...
        if d < bestd:
            best, bestd = c, d
        c += 0.5
    if bestd > d_mid * (1.0 - SYM_GAIN):  # no real gain - trust the midpoint
        best = mid
    return base + best
...
def solve(views, ink, lab, dims):
    """mm-per-pixel for every assigned view, plus an audit of the sheet."""
    active = [v for v in views if v["role"] != "ignore"]
    if not active:
    ...
    for v in active:
        ...
        # Kept so export can size the canvas around it and verify can check it
        # - a silent centring shift was exactly the bug that went unnoticed.
...
# ---------------------------------------------------------------- export ----
def export(views, ink, lab, dims, outdir, px_per_mm=4.0, margin=1.06,
           mode="fit", prefix=""):
    ...
    if not math.isfinite(margin) or margin < 1.0:
        raise SolveError(
            ...
            f"and the edges of your reference get cut off. Try 1.06.")

    rep = solve(views, ink, lab, dims)
    L, W, H = dims.get("L", 0), dims.get("W", 0), dims.get("H", 0)

    # Scale per view first, because the canvas has to be big enough to hold
    # the widest one AFTER centring. Sizing it from the nominal dimensions
    # alone assumes every view sits exactly on its own midpoint, which is not
    # true once a view is centred on its mirror line instead - the overhang
    # then falls off the edge, and only the extents check downstream notices.
    for r in rep["views"]:
        if mode == "uniform":
    ...
    for r in rep["views"]:
        ...
        # here, where px_per_mm can actually be named as the cause.
        if np.asarray(reg).min() >= 200:
            raise SolveError(
                f"At {px_per_mm} px per mm the '{v['role']}' view renders to "
                f"{dw} x {dh} pixels and its linework disappears. Raise px per "
                f"mm - around 4 suits most sheets.")
        canvas = Image.new("L", (CW, CH), 255)
        # PIL maps the crop's full width onto dw, so step through dw/src rather
        # than kh - at small scales the two differ by enough to see.
        ...
        stem = (f"{v['role']}_{v['facing']}" if v["role"] in ("side", "top", "bottom")
                else v["role"])
        ...
        # want_mm  - the dimensions you typed.
        # expect_mm - what THIS mode should actually produce. In 'fit' they are
        #             the same; in 'uniform' they differ by the view's skew, on
        #             purpose. Keeping both lets verify separate a real export
        #             fault from the drawing's own inconsistency.
        written.append(dict(file=name, role=v["role"], facing=v["facing"],
                            want_mm=[dims[r["axis_h"]], dims[r["axis_v"]]],
...
def verify(outdir, meta):
    """Re-measure the finished files off disk. Does not trust export().

    Returns (text, worst_mm, problems). `worst_mm` is the export's own error -
    how far each written file is from what this mode set out to produce - so
    it stays near zero in 'uniform' mode too, where hitting the typed
    dimensions exactly is not the goal. Deviation from the typed dimensions is
    reported alongside it, separately, because the two mean different things.
    """
    P = meta["px_per_mm"]
    ...
    for fn, m in imgs.items():
        ...
        lines.append(f"  {fn:24s} {ah}={gh:8.2f} (want {want[0]:7.2f})   "
                     f"{av}={gv:8.2f} (want {want[1]:7.2f})")

        # Ink on the border means the canvas cut the view off. Extents alone
        # cannot tell you this - a clipped view still reports an extent, just
        # a short one - which is why it needs its own check.
        if xs.min() == 0 or ys.min() == 0 or xs.max() == CW - 1 or ys.max() == CH - 1:
            problems.append(f"{fn} touches the canvas edge - it is being cut "
                            f"off. Raise margin.")

        # Two separate questions, and the difference between them matters.
        #
        # First: did export put the view where solve asked? Deliberate centring
        ...
        if max(abs(dh), abs(dv)) > max(0.5, 0.002 * max(want)):
            problems.append(f"{fn} is off centre by ({dh:+.2f}, {dv:+.2f}) mm.")

        # Second: was that what solve should have asked for? Subtracting the
        # intended offset above makes a wrong mirror line invisible, because
        # the view lands exactly where it was told to. So the size of the
        # intention is checked on its own. A view centred a long way off its
        # own bounding box is either genuinely lopsided or wrongly measured,
        # and last time that went unreported.
        for k, ax in ((0, ah), (1, av)):
            if abs(off[k]) > max(0.5, 0.02 * want[k]):
                problems.append(
    ...
    lines.append(f"  worst deviation from the dimensions you typed: "
                 f"{worst_dim:.2f} mm")

    def corr(a, b, axis, win_px):
        """Best lag within +/-win_px and how strong that peak is.

        The window matters: two views can share an axis yet have quite
        different ink profiles, and an unbounded search will lock onto a
        strong but meaningless peak far from zero. Gross errors are already
        caught by the extents check, so only small lags are of interest.
        """
    ...
    # The extents check above is the authoritative one: it is exact, and it is
    # what actually determines whether the planes agree. What follows looks for
    # small internal drift and is advisory - two views can share an axis and
    # still have quite different ink profiles, in which case it has nothing to
    # say and reports that instead of guessing.
    lines.append("internal drift (advisory - extents above are the real test)")
    names = [n for n in imgs if imgs[n].any()]
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            a, b = names[i], names[j]
            ra, rb = info[a]["role"], info[b]["role"]
            for axa, axb, ax in ((ROLE_AXES[ra][0], ROLE_AXES[rb][0], 0),
                                 (ROLE_AXES[ra][1], ROLE_AXES[rb][1], 1)):
                if axa != axb:
                    continue
...
# its LARGER pixel dimension equal to empty_display_size, the smaller one
# scaled by the aspect ratio. Checked in Blender 5.2 by rendering a 636x1272
# empty at size 0.5 through a 1.0-unit ortho camera - it came out 99 x 199 px
# in a 400 px frame, not 200 x 400. Offset -0.5,-0.5 then centres it on the
# object origin. Every number below depends on that.
...
def blender_script(meta):
    ...
    not need the sheet - just the PNGs sitting beside it.
    """
    CW, CH = meta["canvas_px"]
    ...
    rows = []
    for f in meta["files"]:
        key = (f["role"], f["facing"] if f["role"] in ("side", "top", "bottom") else "n/a")
        b = BLENDER.get(key)
        if b is None:
            continue
...
def build():
    ...
    # Clear a previous run rather than stacking a second set on top. Mixing
    # two exports is the classic way to end up with planes that do not line
    # up, because only the canvas height changes between them.
...
def blender_report(meta):
    ...
    o = ["BLENDER SETUP",
         ...
         f"at {meta['px_per_mm']} px/mm", "",
         "  DO IT FOR ME:  viewforge_import.py, written beside these planes.",
         ...
         "      Depth     Back        Side  Both        Opacity  0.5", "",
