...
canvas, plus a Blender script that builds the planes for you — then re-measures
its own output files and tells you how far off they are.
...
- **It never guesses quietly.** A drawing that is out of square, a view that is
...
**The main window** — a blueprint loaded, views detected, roles assigned, and
the report showing what the numbers imply.

![Editing a box](Assets/Screenshots/editing.png)

**Editing a box** — drag any edge on its own, a corner to move two, or the
middle to move the whole thing.

![In Blender](Assets/Screenshots/blender.png)

**The result in Blender** — planes built by the generated import script,
centred on the world origin and lined up with each other.
...
| **Tkinter** | Ships with Python on Windows and macOS. On Linux, install it separately — see below. |
...
2. **Detect views.** Each blob of ink becomes a candidate region. If the tool
   reports that it merged everything into one region, the blueprint's dimension
   lines are joining the views together — pull the boxes apart by hand, or draw
   your own.
...
Editing an **auto-detected** region converts it to a hand box, and the report
...
had no effect whatsoever. Once it is a box, the rectangle is the measurement.
...
Edges pushed past the edge of the blueprint are pulled back onto it. They cannot
be left there, because an edge hanging off the image would be measured at its
stated position while only the ink inside it actually got used — a scale error
...
object gives the right *picture* and the wrong *scale*; boxing the measured
points gives the right scale and throws away the rest of the view.
...
- They start sitting exactly on the box edges, so switching them on changes
...
- A line that ends up outside its own box is allowed, but you get a warning —
  that is far more often a leftover than an intention.
...
object's front faces the viewer, so the question has no answer; a live dropdown
there would only invite a setting that does nothing.
...
placed box shifts the result less than it would if any single view were trusted.
Measure lines still apply.
...
would. Nothing in the export is a measurement, and the report says so outright
rather than leaving a set of authoritative-looking millimetres lying around.
...
model (wheels, bores); use `fit` when alignment matters more than fidelity to
the drawing.
...
| File | What it is |
|---|---|
| `side_left.png`, `top_up.png`, … | One plane per assigned view, all on the same canvas |
...
> `Size`, checked in Blender 5.2 by rendering a 636×1272 empty at `Size 0.5`
> through a 1.0-unit orthographic camera. It came out 99×199 px in a 400 px
> frame, not 200×400.
...
- **Deviation from the expected extent** — in `fit` mode this matches the above.
  In `uniform` mode it is *expected* to be non-zero: it is the drawing's own
...
that rather than from a fixed number of millimetres, which would otherwise call
a coarse canvas broken while letting real drift through on a fine one — and
which would mean nothing at all in proportional mode.
...
dimension that measures the wrong feature (a slide width when you meant the
overall width) shows up here immediately as a very large percentage. This is
also what a missing measure line looks like.

**"Views disagree on scale by N%"** — the blueprint is not internally
consistent. `fit` mode will still line the planes up by stretching the worst
view that far; `uniform` mode leaves the mismatch visible.
...
Mirror modifier. It is only correct if the view really is symmetric, so it is
stated outright instead of applied silently. The mirror line is looked for on
the object's width axis wherever that lands in the image, so it stays right on a
view drawn a quarter turn round.

**"Touches the canvas edge"** — a plane got cut off. Rare; the canvas normally
grows to prevent it.

## Using the engine without the GUI

`viewforge_core` has no interface and never opens a window, so it can be
imported and driven directly to batch a pile of blueprints.
